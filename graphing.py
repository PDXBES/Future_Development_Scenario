...
horizon_year = utility.calc_horizon_year(config.horizon_in_years)
...
def graph_projections(output_folder):
    ...
    plt.scatter(horizon_year, trendline(horizon_year), color='red', marker='o', label='{} Year Horizon'.format(str(config.horizon_in_years)))
